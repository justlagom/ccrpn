...
/**
 * ClawCloud Run 专用保活 Worker (ccrpn) with Dashboard
 * * * 环境变量 (Environment Variables):
 * - CLOUD_RUN_URL: (必填) ClawCloud Run API URL (例如：https://eu-central-1.run.claw.cloud/api/desktop/getInstalledApps)
 * - AUTH_TOKEN: (必填) 您的 JWT Token (需包含 'Bearer ' 前缀)
 * - LOGIN_COOKIE: (可选) 您的完整 Cookie 字符串
 * * * KV 命名空间绑定 (必填用于日志):
 * - LOG_KV: 绑定名为 LOG_KV 的 KV 命名空间
 */

const CONFIG = {
  VERSION: '1.5.0', // ClawCloud 专用版本号
...
async function keepAlive(env, source = 'Manual') {
  ...
  // 1. 检查关键 Token 和 URL
  if (!env.CLOUD_RUN_URL || !env.AUTH_TOKEN) {
    logs.push(`[${timestamp}] ❌ 错误: 未检测到 CLOUD_RUN_URL 或 AUTH_TOKEN。请配置。`);
    await saveLogs(env, logs, false);
    return { success: false, logs };
  }

  // 2. 请求 ClawCloud Run API (模拟活跃)
  try {
    const start = Date.now();
    const response = await fetch(env.CLOUD_RUN_URL, {
      method: 'GET',
      headers: {
        // 使用 JWT Token
        'Authorization': env.AUTH_TOKEN, 
        // 使用 Cookie
        'Cookie': env.LOGIN_COOKIE || '',
        'User-Agent': 'ClawCloud-KeepAlive-ccrpn/1.5',
    ...
    if (response.ok) {
      logs.push(`[${timestamp}] ✅ CCR API 验证成功 (${duration}ms) - 状态: ${response.status}`);
    } else {
      success = false;
      logs.push(`[${timestamp}] ❌ CCR API 失败: ${response.status} ${response.statusText}`);
      logs.push(`[${timestamp}] ⚠️ 失败原因可能是 AUTH_TOKEN 或 LOGIN_COOKIE 已过期。`);
    }
  } catch (error) {
    success = false;
    logs.push(`[${timestamp}] ❌ CCR API 请求异常: ${error.message}`);
  }
...
// --- 辅助函数：handleTrigger, handleGetLogs, saveLogs (和您的 Koyeb 项目保持一致) ---
...
/**
 * 生成 HTML Dashboard (仅 ClawCloud Run)
 */
function getHtml(env) {
  const hasTokenAndUrl = env.CLOUD_RUN_URL && env.AUTH_TOKEN;
...
<head>
  ...
  <title>ClawCloud Run 保活 Dashboard</title>
...
<body class="min-h-screen flex flex-col items-center py-10 px-4">
  ...
  <div class="w-full max-w-4xl mb-8 flex justify-between items-center">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold text-white tracking-tight">ClawCloud Run 保活助手</h1>
        <p class="text-slate-400 text-sm">Cloudflare Worker 部署版 v${CONFIG.VERSION}</p>
      </div>
    </div>
    <a href="https://github.com/justlagom/ccrpn" target="_blank" class="text-slate-400 hover:text-white transition">
      ...
      <div class="glass rounded-xl p-6 shadow-xl relative overflow-hidden group">
        ...
        <div class="space-y-4">
          <div class="flex justify-between items-center">
            <span class="text-slate-300">凭证配置 (URL + Token)</span>
            ${hasTokenAndUrl 
              ? '<span class="px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded border border-green-500/30">已配置</span>' 
              : '<span class="px-2 py-1 bg-red-500/20 text-red-400 text-xs rounded border border-red-500/30">缺少凭证</span>'}
          </div>
          <div class="flex justify-between items-center">
            ...
            ${hasKV 
              ...
              : '<span class="px-2 py-1 bg-red-500/20 text-red-400 text-xs rounded border border-red-500/30">未绑定 KV</span>'}
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    const hasKV = ${hasKV};
    const hasConfig = ${hasTokenAndUrl};
    ...
    async function loadLogs() {
      if (!hasKV) {
          ...
          emptyState.innerText = "未绑定 KV 数据库，无法加载历史日志";
          return;
      }
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          emptyState.style.display = 'flex';
          emptyState.innerText = "暂无历史记录";
        }
      } catch (e) {
        console.error(e);
        emptyState.style.display = 'flex';
        emptyState.innerText = "日志加载失败：" + e.message;
      }
    }
    ...
    async function triggerKeepAlive() {
      if (!hasConfig) {
         alert('错误：请先配置 CLOUD_RUN_URL 和 AUTH_TOKEN 环境变量！');
         return;
      }
